...
import { FiCpu, FiEdit, FiLayout, FiPlay, FiShare2, FiDownload, FiLogOut, FiPenTool } from 'react-icons/fi';
...
export const Header = ({ view, setView, onShare, onPresent, onLogout }) => {
    ...
    const slidesExist = usePresentationStore(state => state.slides.length > 0);
        ...
        <header className="flex items-center justify-between p-4 border-b border-white/10 bg-black/30 backdrop-blur-sm flex-shrink-0">
          ...
          <nav className="flex items-center gap-1 p-1 rounded-full bg-black/30 border border-white/10">
            {[
                ...
                {name:'Design',icon:FiPenTool,key:'design'} // Design mode is fully enabled
            ].map(item =>(
              <motion.button 
                key={item.key} 
                onClick={()=>setView(item.key)} 
                className={`relative px-4 py-2 text-sm font-medium rounded-full transition-colors duration-300 ${view===item.key?'text-white':'text-gray-400 hover:text-white'} ${(!slidesExist && item.key !== 'idea') || item.disabled ? 'cursor-not-allowed opacity-50' : ''}`}
                disabled={(!slidesExist && item.key !== 'idea') || item.disabled}
